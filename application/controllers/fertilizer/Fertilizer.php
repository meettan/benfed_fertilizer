...
public function productAdd(){
		...
		if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				$product['prod_type']    = $this->FertilizerModel->f_select('mm_product_type',NULL,NULL,0);
				$this->load->view('post_login/fertilizer_main');
...
public function sale_rate(){
	...
	$where      =   array(
		...
		// $ro   = $this->FertilizerModel->f_select('td_	 a,mm_company_dtls b',$select,$where,0);
...
public function salerateAdd(){
	...
	if($_SERVER['REQUEST_METHOD'] == "POST") {
		   ...
		   $fpo_gst    = $this->input->post('fpo_gst');
		   $fpo_wtoutgst = $this->input->post('fpo_wtoutgst');
