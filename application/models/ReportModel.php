...
/////////
    public function p_ro_wise_prof_calc_all_comp_pro_dist($fdate, $tdate, $comp, $product_id, $district_id)
    {
        ...
        and a.ro_no not in (select sale_ro from td_sale 
                         where do_dt between "' . $fdate . '" and "' . $tdate . '"
                         and   comp_id = ' . $comp . ')');
        }else if($district_id == 0 && $product_id == 0){
            $q = $this->db->query('SELECT c.short_name comp_name,
        d.prod_desc,
        a.ro_no,
        a.ro_dt,
  ...
  union
         ...
         SELECT trans_dt,'' prod,recpt_no as inv_no, c.soc_id soc_id,soc_name,sum(c.tot_amt) as paid_amt,0 paybl,0,0,c.ro as ro_no,trans_dt as ro_dt,0 as qty ,0,'Cr note' remarks
            FROM tdf_dr_cr_note c,mm_ferti_soc b,td_sale d where c.soc_id=b.soc_id and c.soc_id = '$soc_id'and c.branch_id='$branch' and c.invoice_no = d.trans_do and c.trans_flag='R' and c.trans_dt between '$frmDt' and '$toDt' and c.catg!=7
...
///************************* */
function outstanding_list_model($frmdate,$todate,$comp_id)     
    {
...
function outstanding_list_model_br($frmdate,$todate,$comp_id,$br)     
    {
        ...
        '' AS trans_do,
        a.trans_dt,
        DATEDIFF(g.sale_due_dt, g.do_dt) AS no_of_days,
    ...
    WHERE 
        a.trans_dt BETWEEN '2026-04-01' AND '2026-07-23'
        AND a.comp_id = 3
        AND a.adv_status = 'N'
        AND  a.br = '335'
    GROUP BY  
