...
public function f_get_prodcompwisesale($frm_dt, $to_dt)
    {
        $query  = $this->db->query("select  prod_desc,
        sum(IFFCO_QTY)IFFCO_QTY,sum(IFFCO_VALUE)IFFCO_VALUE,
        sum(KRIBCO_QTY)KRIBCO_QTY,sum(KRIBCO_VALUE)KRIBCO_VALUE,
        sum(IPL_QTY)IPL_QTY, sum(IPL_VALUE)IPL_VALUE,
        sum(CIL_QTY)CIL_QTY,sum(CIL_VALUE)CIL_VALUE,
        sum(KCFL_QTY)KCFL_QTY,sum(KCFL_VALUE)KCFL_VALUE,
        sum(JCF_QTY)JCF_QTY,sum(JCF_VALUE)JCF_VALUE,
        sum(MIPL_QTY)MIPL_QTY,sum(MIPL_VALUE)MIPL_VALUE,
        sum(CCFL_QTY)CCFL_QTY,sum(CCFL_VALUE)CCFL_VALUE,
        sum(HURL_QTY)HURL_QTY,sum(HURL_VALUE)HURL_VALUE,
        sum(KFL_QTY)KFL_QTY,sum(KFL_VALUE)KFL_VALUE,
        sum(MFCL_QTY)MFCL_QTY,sum(MFCL_VALUE)MFCL_VALUE,
        sum(NFL_QTY)NFL_QTY,sum(NFL_VALUE)NFL_VALUE
                                    from(
                                    SELECT b.prod_desc,if(c.comp_id=1,round(sum(CASE
                                    WHEN a.unit = 1 THEN a.qty
                                    WHEN a.unit = 2 THEN a.qty/1000
                                    WHEN a.unit = 4 THEN a.qty/10
                                    WHEN a.unit = 6 THEN a.qty/10000
                            ...
                            if(c.comp_id=8,round(sum(CASE
                                ...
                                END ),3),0)MFCL_LQQTY,if(c.comp_id=9,sum(a.tot_amt) ,0)MFCL_VALUE
                                    ...
                                    WHERE a.do_dt between $frm_dt and $to_dt
                                    AND   a.prod_id=b.prod_id
                                    AND a.comp_id=b.company
                                    AND c.comp_id=a.comp_id
                                    GROUP by b.prod_desc,c.comp_id)a
                                    group by prod_desc;");
...
///////////////////////////////
public function p_ro_wise_prof_calc_allbr($fdate, $tdate, $comp)
    {
...
GROUP BY 
    ...
    public function getallAdvfwdData($comp_id, $frm_date, $to_date, $branch_id)
    {
        ...
        if ($branch_id == 0) {
            ...
            and   c.prod_id = d.PROD_ID
            and   a.comp_id = e.comp_id
            and   c.detail_receipt_no = f.detail_receipt_no
            and   a.trans_dt between '$frm_date' and '$to_date'
            and   a.comp_id = '$comp_id'
            and   c.comp_pay_flag = 'Y'
    ...
    ////***/
    public function getadvCompanyPayment($comp_id, $frm_date, $to_date, $refereceNo=null)
    {
        $fin_id = $this->session->userdata['loggedin']['fin_id'];
    ...
    ////**** */ */
    public function getpaymentsummary($comp_id, $frm_date, $to_date, $refereceNo)
    {
        $fin_id = $this->session->userdata['loggedin']['fin_id'];
    ...
    /***** */
    public function getpayadvummary($comp_id, $frm_date, $to_date, $refereceNo)
    {
        $fin_id = $this->session->userdata['loggedin']['fin_id'];
    if($refereceNo==null||$refereceNo==""){
     ...
     where a.comp_id=$comp_id
     and b.PROD_ID=z.prod_id
     and a.adv_receive_no=z.detail_receipt_no
     and a.branch_id=c.district_code
     and a.comp_id=d.comp_id
     and a.bank=t.sl_no
     and d.comp_id=x.comp_id
     and x.comp_id=z.comp_id
     and b.prod_type=x.cat_id
     and b.prod_type=y.id
     and a.trans_dt >= '$frm_date' and a.trans_dt <= '$to_date'
     AND a.fin_yr = $fin_id
   ...
   // ====================================active society===============================
    ...
    public function tcs_payable($frm_date,$to_date,$op_dt){
        ...
        (select sum(IFNULL(b.adv_amt,0)) from tdf_advance b
         ...
         and b.trans_type='I'
         group by b.soc_id)
         + (select IFNULL(sum(x.paid_amt),0)FROM tdf_payment_recv x
         WHERE x.paid_dt BETWEEN '$frm_date' and '$to_date'
        AND x.branch_id = a.br_cd 
          and x.soc_id=c.soc_id
          and x.pay_type not in(6,2))adv_amt,
        ...
        AND x.branch_id = a.br_cd 
          and x.soc_id=c.soc_id
          and x.pay_type not in(6,2))adv_amt_upto
